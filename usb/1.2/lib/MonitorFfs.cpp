...
MonitorFfs::MonitorFfs(void)
    ...
    std::string UsbController = GetProperty(kUSBController, "");

    if (UsbController.size() == 0) {
        ALOGE("%s property not found", kUSBController);
        abort();
    }
    mGadgetName = UsbController;
}
...
std::string MonitorFfs::gadgetName() {
    return mGadgetName;
}
